...
public interface BaseEntity<T extends BaseEntity<T>> {
    ...
    default T push() {
        if (id() == null) {
            return (T) RhesisClient.getDefault().getHttpClient().post(getEndpointPath() + "/", this, getClass());
        } else {
            // Update using PUT
            return (T) RhesisClient.getDefault().getHttpClient().put(getEndpointPath() + "/" + id(), this, getClass());
        }
    }
}
