...
public class ProjectClient {
  ...
  public Project findByName(String name) {
    ...
    String encodedName = URLEncoder.encode(name.toLowerCase(), StandardCharsets.UTF_8).replace("+", "%20");
    String filter = "?$filter=tolower(name)%20eq%20'" + encodedName + "'";

    List<Project> projects = httpClient.get("/projects/" + filter, new TypeReference<List<Project>>() {});
    ...
    if (projects.size() > 1) {
      ...
      throw new IllegalArgumentException("More than one entity found with name '" + name +
          "'. Entity names must be unique. Please use the entity id instead. Matching entity IDs: " + String.join(", ", ids));
    }
